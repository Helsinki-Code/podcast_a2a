...
// Fetches a public URL, validating every redirect hop so a public page cannot bounce the server
// onto a private address.
export async function publicFetch(input, options = {}, maxRedirects = 5) {
  let target = await assertPublicHttpUrl(input);
  for (let hop = 0; hop <= maxRedirects; hop++) {
    const response = await fetch(target, { ...options, redirect: 'manual' });
    if (![301, 302, 303, 307, 308].includes(response.status)) return Object.assign(response, { finalUrl: target });
    const location = response.headers.get('location');
    if (!location) return Object.assign(response, { finalUrl: target });
