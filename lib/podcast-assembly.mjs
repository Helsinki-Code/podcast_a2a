...
import { podcastCaptions, podcastCaptionFilter } from './podcast-timeline.mjs';
import { inspectSandboxMedia, evaluateMediaQuality } from './media-quality.mjs';
import { stageHtml, titleCardHtml, generatedMusicSource, titleMusicFilter, finalAudioGraph, SPEECH_TRIM_FILTER, interruptionFadeFilter, INTRO_SECONDS, OUTRO_SECONDS } from './podcast-media.mjs';
import { castRoles, roleAccent } from './cast.mjs';
import { packageVideo } from './publish-media.mjs';
import { podcastExportTimeline } from './publishing.mjs';

// Plain rendering helpers shared by the render and dubbing steps. Kept out of the step files so the
// workflow bundle, which imports those files, never pulls in the store or Node-only modules.
...
export async function assemblePodcast(browser, item, sourceTimeline, { stem, variant = false }) {
  ...
  const burnCaptions = captionsEnabled && await supportsBurnedCaptions(browser);
  const finalFilters = [...(burnCaptions ? [podcastCaptionFilter(settings.captionStyle, '/tmp/podcast-burn.srt', captionOptions)] : []), ...(width !== 1920 ? [`scale=${width}:${height}:flags=lanczos`] : [])];
  ...
  return { mp4, ...(webm ? { video: webm } : {}), captions: captionUrl, captionsBurned: burnCaptions, videoError: webmError, quality, output: { width, height, format }, timeline: exportTimeline, duration: cursor, ...(packaged ? { chapters: packaged.chapters, youtube: packaged.youtube, thumbnail: packaged.thumbnail, mp3: packaged.mp3 } : {}) };
}
