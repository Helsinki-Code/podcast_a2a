...
export function ownContext(episode, role, agent, screen, mode, extra = '') {
  ...
  const guidance = mode === 'interrupt' ? 'You are listening to the other persona in progress. Decide whether to interrupt now. Return JSON {"interrupt":boolean,"reason":"short"}. Interrupt for a timely, meaningful reaction, direct address, strong agreement or disagreement. Avoid interrupting habitually.' : mode === 'waiting' ? 'A tool is running. Say one brief live reaction, question, or observation to keep the conversation moving. Return JSON {"segments":[{"type":"speak","text":"..."}]}.' : `Continue this live, unscripted podcast from the actual preceding exchange. Return JSON with {"segments":[{"type":"speak","text":"..."},{"type":"act","tool":"one of ${toolNames.join(', ')}","input":{...}}],"finish":boolean}. Generate 1–2 natural speech segments of roughly 8–22 words each. Use short reactions, follow-up questions, occasional fillers, and direct responses instead of monologues. Speak before an action. After every tool action you will be called again with its actual result, so do not prewrite a reaction to output you have not seen. Do not plan or reveal future turns. The host may finish when the discussion reaches a natural close; the guest should never finish the episode. ${demoGuidance} Tool schemas: code {language:"python"|"javascript",code:string}; browser {url:string,action:"visit"|"click"|"fill",selector?:string,value?:string}; diagram {title:string,nodes:[{id,label}],edges:[{from,to}]}; file {name:string,content:string}; play_audio {path:"/tmp/file.wav"} for audio you created in the episode sandbox.`;
