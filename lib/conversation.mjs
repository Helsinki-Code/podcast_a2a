...
export function ownContext(episode, role, agent, screen, mode, extra = '') {
  ...
  const demoGuidance = role === 'guest' && episode.settings.requireGuestDemo && !guestDemoDone ? `This episode requires a real live computer demonstration. In this response, speak briefly and then use the browser tool. ${demo.url ? `Demonstrate ${demo.url}.` : 'Choose a relevant official public web page for the subject.'} ${demo.brief || ''} Use the visible accessibility snapshot and stable @e references for clicks and fills. Never ask for, reveal, or repeat login credentials; any required login was completed before the episode.` : '';
