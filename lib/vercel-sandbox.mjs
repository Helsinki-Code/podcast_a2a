...
export class VercelEpisodeSandbox {
  ...
  async capture(existingSandbox = null, fallbackUrl = '') {
    ...
    const pageUrl = currentUrl.stdout.trim() || fallbackUrl;
    return { type: 'browser', title: pageUrl, content: `${title.stdout.trim()}\n${tree.stdout}`.slice(0, 10000), image };
  }
  async command(args) {
    const sandbox = await this.ensure();
    const { runAgentBrowserCommand } = await import('@agent-browser/sandbox/vercel');
  ...
  async login(demo, credentials) {
    ...
    const script = `import fs from 'node:fs';import{spawnSync}from'node:child_process';const d=JSON.parse(fs.readFileSync(process.argv[2],'utf8'));const a=['--session',d.session,'auth','save',d.profile,'--url',d.url,'--username',d.username,'--password-stdin','--username-selector',d.usernameSelector,'--password-selector',d.passwordSelector,'--submit-selector',d.submitSelector];const r=spawnSync('agent-browser',a,{input:d.password,encoding:'utf8'});if(r.status!==0){process.stderr.write(r.stderr||'Authentication setup failed');process.exit(r.status||1)}`;
    const secret = { session: this.session, profile, ...demo, username: credentials.username, password: credentials.password };
    ...
    try {
      ...
      if (saved.exitCode !== 0) throw new Error((await saved.stderr()).slice(-1800));
