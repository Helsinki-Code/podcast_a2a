...
export class VercelEpisodeSandbox {
  ...
  async locateLoginPage(demo) {
    ...
    if (!demo?.loginUrl) {
      const selector = JSON.stringify(String(demo?.usernameSelector || 'input[type="email"], input[name="email"], input[name="username"]'));
      const passwordSelector = JSON.stringify(String(demo?.passwordSelector || 'input[type="password"]'));
  ...
  async login(demo, credentials) {
    ...
    const secret = { session: this.session, profile, ...demo, authUrl, username: credentials.username, password: credentials.password };
