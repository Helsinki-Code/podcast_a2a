...
const AGENT_BROWSER_STATE_SUFFIXES = ['sock', 'pid', 'version', 'config', 'stream', 'engine'];
export function agentBrowserCleanupScript(session) {
  const safe = String(session || '').replace(/[^A-Za-z0-9_-]/g, '').slice(0, 64);
  return `session=${safe}\nfor base in "$HOME/.agent-browser" "\${AGENT_BROWSER_SOCKET_DIR:-}" "\${XDG_RUNTIME_DIR:+$XDG_RUNTIME_DIR/agent-browser}"; do\n  [ -n "$base" ] || continue\n  pidfile="$base/$session.pid"\n  if [ -f "$pidfile" ]; then\n    pid=$(cat "$pidfile" 2>/dev/null || true)\n    case "$pid" in ''|*[!0-9]*) ;; *) ps -p "$pid" -o args= 2>/dev/null | grep -q agent-browser && kill "$pid" 2>/dev/null || true ;; esac\n  fi\n  ${AGENT_BROWSER_STATE_SUFFIXES.map(suffix => `rm -f "$base/$session.${suffix}"`).join('\n  ')}\ndone`;
}
...
pgrep -x Xvnc >/dev/null && cdp && exit 0
if ! pgrep -x Xvnc >/dev/null; then
  rm -f /tmp/.X99-lock /tmp/.X11-unix/X99
  Xvnc :99 -geometry 1920x1080 -depth 24 -SecurityTypes None -AlwaysShared -rfbport 5900 >/tmp/xvnc.log 2>&1 &
  sleep .5
fi
# The display counts as up once its socket exists (xdpyinfo is optional on the snapshot). Only a
# display server that exited is fatal; one that is running but slow to report is given the benefit.
display_up() { [ -S /tmp/.X11-unix/X99 ] || { command -v xdpyinfo >/dev/null 2>&1 && xdpyinfo -display :99 >/dev/null 2>&1; }; }
for i in $(seq 1 40); do
  display_up && break
  if [ "$i" -gt 8 ] && ! pgrep -x Xvnc >/dev/null; then break; fi
  sleep .25
done
if ! pgrep -x Xvnc >/dev/null; then sleep .5; echo 'The virtual display did not start' >&2; tail -n 20 /tmp/xvnc.log >&2 2>/dev/null; exit 1; fi
...
export class VercelEpisodeSandbox {
  ...
  async resetAgentBrowser(sandbox) {
    await sandbox.runCommand('sh', ['-lc', agentBrowserCleanupScript(this.session)]);
    // Recent agent-browser releases also repair stale daemon metadata through doctor. Older
    // snapshot versions may not expose doctor yet, so this remains a best-effort supplement.
    await sandbox.runCommand('agent-browser', ['doctor', '--offline', '--quick', '--json']).catch(() => {});
  }
  async connectAgentBrowser(sandbox, connectedMarker) {
    let lastError;
    for (let attempt = 0; attempt < 3; attempt++) {
      if (attempt) {
        await this.resetAgentBrowser(sandbox);
        await delay(250 * attempt);
      }
      try {
        await this.rawAgentCommand(sandbox, ['connect', `http://127.0.0.1:${CDP_PORT}`]);
        await this.rawAgentCommand(sandbox, ['get', 'url']);
        await sandbox.runCommand('touch', [connectedMarker]);
        return;
      } catch (cause) { lastError = cause; }
    }
    const diagnostic = await sandbox.runCommand('sh', ['-lc', `agent-browser --version 2>&1; node --version 2>&1; ls -la "$HOME/.agent-browser" 2>&1 | tail -n 30`]).catch(() => null);
    const detail = diagnostic ? `${await diagnostic.stdout()}\n${await diagnostic.stderr()}`.trim().slice(-1800) : '';
    throw new Error(`The Computer Use browser controller could not reconnect after clearing its stale daemon state: ${lastError?.message || lastError}${detail ? `\nDiagnostics:\n${detail}` : ''}`, { cause: lastError });
  }
  async ensureDesktop(existingSandbox = null) {
    ...
    if (check.exitCode !== 0) {
      // /tmp persists when a named Vercel Sandbox is stopped and resumed, but its processes do
      // not. A marker from the previous boot must never imply that its daemon is still alive.
      await sandbox.runCommand('rm', ['-f', connectedMarker]);
    ...
    let healthy = false;
    if (connected.exitCode === 0) healthy = await this.rawAgentCommand(sandbox, ['get', 'url']).then(() => true, () => false);
    if (!healthy) {
      await sandbox.runCommand('rm', ['-f', connectedMarker]);
      await this.connectAgentBrowser(sandbox, connectedMarker);
    }
    return sandbox;
  }
