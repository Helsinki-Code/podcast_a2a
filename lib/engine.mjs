import { modelProviders, speechProviders, supportedVoice } from './providers.mjs';
...
import { demoLeadInComplete, ownContext, speechBlocks, speechPhrases } from './conversation.mjs';
import { episodePlanQualityIssue } from './episode-plan.mjs';
...
export async function runEpisode(episode, publish, waitForAck) {
  ...
  try {
    ...
    while (!state.stop && Date.now() - started < maxMinutes * 60000) {
      ...
      const qualityIssue = episodePlanQualityIssue(plan, { role, mode: 'turn' });
      if (qualityIssue) throw new Error(`${role} ${qualityIssue}.`);
      ...
      if (role === 'guest' && episode.settings.requireGuestDemo && demoLeadInComplete(episode) && !guestDemoDone && !segments.some(segment => segment.type === 'act' && segment.tool === 'browser')) {
      ...
      for (const segment of segments) {
        ...
        if (segment.type === 'speak') {
          ...
          if (!text) continue;
          const phrases = (role === 'guest' ? speechBlocks : speechPhrases)(text);
          for (let i = 0; i < phrases.length; i++) {
            ...
            const audio = startSpeech(speech, phrase, supportedVoice(providerName, agent.voice, role === 'host' ? 'coral' : 'nova'));
          ...
          if (name === 'browser' && episode.settings.requireGuestDemo && !demoLeadInComplete(episode)) continue;
          ...
          if (!settled && !state.stop) {
            try {
              ...
              if (line) {
                ...
                episode.turns.push(turn);
                const otherProvider = other.speechProvider || 'gateway';
                const audio = startSpeech(speechProviders.get(otherProvider), line, supportedVoice(otherProvider, other.voice, otherRole === 'host' ? 'coral' : 'nova'));
