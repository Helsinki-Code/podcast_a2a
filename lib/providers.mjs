...
registerModel('gateway', {
  ...
  async generateVisual(messages, image, model) {
    const { generateText, Output, jsonSchema } = await import('ai');
    const modelId = model || process.env.AI_GATEWAY_COMPUTER_MODEL || process.env.AI_GATEWAY_MODEL || 'openai/gpt-4o-mini';
    ...
    if (!output || typeof output !== 'object' || Array.isArray(output)) throw new Error('AI Gateway returned an invalid computer-use plan.');
    return output;
  }
});
