...
export function podcastCaptionFilter(style = 'studio', subtitlePath = '/tmp/podcast-burn.srt', options = {}) {
  ...
  return `subtitles=filename='${subtitleFilename(subtitlePath)}':force_style='FontName=${fonts[options.font] || fonts.sans},FontSize=${size},Bold=1,PrimaryColour=${look.primary},OutlineColour=${look.outline},BackColour=${look.back},BorderStyle=${look.border},Outline=${look.outlineWidth},Shadow=${look.shadow},Alignment=${alignment},MarginL=40,MarginR=40,MarginV=${alignment === 5 ? 0 : 14}'`;
}
