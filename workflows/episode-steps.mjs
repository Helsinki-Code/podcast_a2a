...
export async function publishSpeech(episodeId, role, text, eventId, prepared, acknowledged = false) {
  'use step';
  const turn = { id: uid(), at: stamp(), role, text };
  return emit(episodeId, 'speech', { role, text, audio: prepared.audio, turnId: turn.id, ...(acknowledged ? { acknowledged: true, background: true } : {}) }, turn, eventId);
}
