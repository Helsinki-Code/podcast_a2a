...
try {
  ...
  const captions = buildCaptions([{ text: 'The cursor moves naturally to the link while narration explains the destination.', duration: 5.5, captionDuration: 4.4 }, { text: 'The agent types into the form with visible human paced keystrokes.', duration: 4, captionDuration: 3.2 }, { text: 'A corrupt capture recovers without turning the subtitle track into a paragraph.', duration: 2.2 }], { wordsPerCue: 5 });
  await browser.writeSandboxFile('/tmp/check.srt', captions);
  result = await browser.run('ffmpeg', ['-y', '-f', 'lavfi', '-i', 'anullsrc=r=48000:cl=stereo', '-t', '11.7', '-c:a', 'aac', '/tmp/check.m4a']);
  if (result.exitCode) throw new Error(await result.stderr());
